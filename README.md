...
Zenodo 存档提供以下 3 个压缩文件：

| 文件 | 大小 | 说明 |
|------|------|------|
| `data_ready.zip` | ~6 GB | 就绪数据，**通常只需下载此文件** |
| `data_raw_interm.zip` | ~11 GB | 原始数据与中间缓存（如需查看预处理过程） |
| `results.zip` | ~1 GB | v1 初版旧结果（无需下载，建议通过新版代码重新生成） |

下载后将压缩包直接解压到仓库根目录（注意不要让解压工具额外创建子目录），文件将自动落入 `data/` 对应子目录。解压完成后可运行以下命令确认：

```bash
ls data/ready/ | wc -l   # 应输出 16（含仓库已有的 sfi_1975-2024.csv）
```
